...
import { useUpdateUserSocialEventMutation, usePresignUserEventImageMutation, useGetUserSocialEventQuery } from "../../services/social";
...
export default function EditEventPage() {
  ...
  const { data: eventRes, isLoading: isLoadingEvent } = useGetUserSocialEventQuery(id || '', {
    skip: !id,
  });
  ...
  // Populate form with event data when fetched
  useEffect(() => {
    if (eventData && eventData.id) {
      const startsAt = eventData.startsAt ? new Date(eventData.startsAt) : null;
      const endsAt = eventData.endsAt ? new Date(eventData.endsAt) : null;
      
      setFormData({
        id: eventData.id || '',
        title: eventData.title || '',
        startDate: startsAt ? startsAt.toISOString().split('T')[0] : '',
        startTime: formatUtcTime(eventData.startsAt),
        endDate: endsAt ? endsAt.toISOString().split('T')[0] : '',
        endTime: formatUtcTime(eventData.endsAt),
        eventType: eventData.eventType?.toLowerCase() || '',
        socialChapterId: eventData.socialChapterId || mySocialChapterId || '',
        country: eventData.countryId || myCountryId || '',
        region: eventData.regionId || myRegionId || '',
        category: eventData.categoryLabel?.toLowerCase() || '',
        contactPerson: eventData.contactPerson || '',
        costForMember: eventData.costForMembers?.toString() || '',
        maxAttendees: eventData.maxAttendees?.toString() || '',
        link: eventData.link || '',
        description: eventData.description || '',
        eventPhoto: null,
        existingImageUrl: eventData.imageUrl || undefined,
        status: eventData.status || 'pending'
      });
    }
  }, [eventData, mySocialChapterId, myCountryId, myRegionId]);
  ...
  const [updateEvent] = useUpdateUserSocialEventMutation();
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!id) {
      showToast({ title: "Error", description: "Event ID is missing.", kind: "error" });
      return;
    }
    ...
    try {
      let imageKey: string | undefined = formData.existingImageUrl;

      // Upload new image to S3 via presigned URL if a new file was selected
    ...
    <SocialLayout>
      <div className="p-[3px] rounded-xl bg-gradient-to-br from-white/30 via-white/5 via-transparent to-white/1">
        <div className="bg-[#161b22] rounded-[9px] p-8">
          <h2 className="text-white text-xl font-semibold mb-6">Edit Event</h2>
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
              ...
              <div className="w-full">
                ...
                <div className="relative">
                  ...
                  >
                    <span className="truncate max-w-[calc(100%-3rem)]">
                      {formData.eventPhoto ? formData.eventPhoto.name : (formData.existingImageUrl ? "Current image" : "Choose File")}
                    </span>
                ...
                {/* Image Preview */}
                {(formData.existingImageUrl || formData.eventPhoto) && (
                  <div className="mt-3">
