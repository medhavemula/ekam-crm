...
export default function SocialAdminAllEvents() {
  // ---------------- FILTER STATES ----------------
  ...
  const regionMap = useMemo(() => {
    const map = new Map<string, { name: string; countryId?: string }>();
    regions.forEach((r: any) => {
      map.set(r.id, { name: r.name, countryId: r.countryId });
    });
    return map;
  }, [regions]);
  ...
  const dateGroups: DateGroup[] = useMemo(() => {
    ...
    data.data.items.forEach((item: any) => {
      ...
      // Format the full dateTime using UTC to match detail page
      ...
      const chapterRegionId =
        chapters.find((ch: SocialChapterItem) => ch.id === item.socialChapterId)?.regionId ?? undefined;

      const region = item.regionId
        ? regionMap.get(item.regionId)
        : chapterRegionId
          ? regionMap.get(chapterRegionId)
          : undefined;

      const countryName = region?.countryId
        ? countryMap.get(region.countryId)
        : "";
  ...
  }, [data, chapters, regionMap, countryMap]);
                ...
                {group.events.map((event) => (
                  <GradientContainer key={event.id} className="p-0">
                    <div
                      ...
                      onClick={() => {
                        console.log("Navigating from All Events with pageType: all-events");
                        navigate(`/social/admin/event-details/${event.id}`, {
                          state: { pageType: "all-events", from: "all-events" },
                        });
